					...
					<div class="card">
						...
						<div class="card-footer">
							...
							<?php if ($this->session->userdata('level') == 1 && $data->id_approval == 2) { ?>
								<a onclick="return confirm('Apakah Anda yakin menyelesaikan request support ini?')" id="selesaikan" class="btn btn-info" href="<?= base_url('Admin_nst/complete/nst/id/' . $data->id_nst) ?>">Selesaikan</a>
							<?php } ?>
							<!-- Tombol Aksi ini akan muncul untuk Admin NST -->
							<?php if (($this->session->userdata('level') == 5 || $this->session->userdata('level') == 4)) { ?>
								<a onclick="return confirm('Apakah Anda yakin menyetujui request support ini?')" class="btn btn-info" href="<?= base_url('Admin_nst/approve/nst/id/' . $data->id_nst) ?>">Approve</a>
								<a onclick="return confirm('Apakah Anda yakin menyetujui request support ini?')" class="btn btn-danger" href="<?= base_url('Admin_nst/reject/nst/id/' . $data->id_nst) ?>">Reject</a>
							<?php } ?>
						</div>
					</div>
				</div>
