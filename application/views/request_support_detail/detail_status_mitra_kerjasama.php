					...
					<div class="card">
						...
						<div class="card-footer">
							...
							<?php if ($this->session->userdata('level') == 3 && $data->id_approval == 2) { ?>
								<a class="btn btn-info" onclick="return confirm('Apakah Anda yakin MENYELESAIKAN request support ini?')" href="<?= base_url('Aksi/complete/mitra_kerjasama/id/' . $data->id_mitra_kerjasama) ?>">Approve</a>
								<a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/mitra_kerjasama/id/' . $data->id_mitra_kerjasama) ?>">Reject</a>
							<?php } ?>
							<!-- Tombol Aksi ini akan muncul untuk Admin Superuser -->
							<?php if ($this->session->userdata('level') == 5) { ?>
								<a class="btn btn-info" href="<?= base_url('Aksi/complete/mitra_kerjasama/id/' . $data->id_mitra_kerjasama) ?>">Complete</a>
								<a class="btn btn-danger" href="<?= base_url('Aksi/reject/mitra_kerjasama/id/' . $data->id_mitra_kerjasama) ?>">Reject</a>
							<?php } ?>
						</div>
					</div>
				</div>
