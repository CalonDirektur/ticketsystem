				...
				<div class="col-lg-6 col-md-6">
					<div class="card">
						...
						<div class="card-body">
							...
							<?php if (($this->session->userdata('level') == 1 || $this->session->userdata('level') == 5 || $this->session->userdata('level') == 6)  && ($data->id_approval == 0 || $data->id_approval == 1)) { ?>
						...
						<div class="card-footer">
							...
							<?php if ($this->session->userdata('level') == 3 && $data->id_approval == 2) { ?>
								<a class="btn btn-info" onclick="return confirm('Apakah Anda yakin MENYELESAIKAN request support ini?')" href="<?= base_url('Aksi/complete/aktivasi_agent/id/' . $data->id_agent) ?>">Approve</a>
								<a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/aktivasi_agent/id/' . $data->id_agent) ?>">Reject</a>
							<?php } ?>
							<!-- Tombol Aksi ini akan muncul untuk Admin Superuser -->
							<?php if ($this->session->userdata('level') == 5) { ?>
								<a class="btn btn-info" href="<?= base_url('Aksi/complete/aktivasi_agent/id/' . $data->id_agent) ?>">Complete</a>
								<a class="btn btn-danger" href="<?= base_url('Aksi/reject/aktivasi_agent/id/' . $data->id_agent) ?>">Reject</a>
							<?php } ?>
						</div>
					</div>
				</div>
