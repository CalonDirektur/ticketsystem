      ...
      <div class="row">
        ...
        <div class="col-lg-6">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <!-- ID Vendor -->
              <input type="hidden" class="form-control id_vendor" name="id_vendor_lainnya" id="id_vendor_lainnya" value="<?= $data->id_vendor ?>" readonly required>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  <input name="nama_penyedia_jasa" type="text" class="form-control nama_vendor_myhajat" placeholder="Nama Vendor" value="<?= $data->nama_penyedia_jasa ?>" readonly required>
                  <div class="input-group-append">
                    <button type="button" class="btn btn-danger enable clear-nama-vendor" disabled>x</button>
                  </div>
                </div>
              </div>
              <!-- Jenis Vendor -->
              <div class="form-group jenis-vendor">
                <label for="jenis_penyedia_jasa">Jenis Penyedia </label>
                <div class="form-check form-check-info">
                  <label class="form-check-label"><input class="kategori form-check-input perorangan" type="radio" name="jenis_penyedia_jasa" value="Perorangan" <?= $data->jenis_penyedia_jasa == 'Perorangan' ? 'checked' : '' ?> disabled required>Perorangan</label>
                </div>
                <div class="form-check form-check-info">
                  <label class="form-check-label"><input class="kategori form-check-input badan_usaha" type="radio" name="jenis_penyedia_jasa" value="Badan Usaha" <?= $data->jenis_penyedia_jasa == 'Badan Usaha' ? 'checked' : '' ?> disabled required>Badan Usaha</label>
                </div>
              </div>
              <div class="form-group">
            ...
            <div class="card-footer">
              ...
              <?php if ($this->session->userdata('level') == 2 && ($data->id_approval == 0 || $data->id_approval == 4)) { ?>
                ...
                <a class="btn btn-info" onclick="return confirm('Apakah Anda yakin menyetujui request support?')" href="<?= base_url('Aksi/approve/myhajat/lainnya/' . $data->id_myhajat_lainnya) ?>">Approve</a>
                <a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/myhajat/lainnya/' . $data->id_myhajat_lainnya) ?>">Reject</a>
              <?php } ?>
              <!-- Tombol Aksi ini muncul untuk Admin 2 (Mas Gede) -->
              <?php if ($this->session->userdata('level') == 3 && $data->id_approval == 2) { ?>

                <a class="btn btn-info" onclick="return confirm('Apakah Anda yakin MENYELESAIKAN request support ini?')" href="<?= base_url('Aksi/complete/myhajat/lainnya/' . $data->id_myhajat_lainnya) ?>">Approve</a>
                <a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/myhajat/lainnya/' . $data->id_myhajat_lainnya) ?>">Reject</a>
              <?php } ?>
              <!-- Tombol Aksi ini akan muncul untuk Admin Superuser -->
              <?php if ($this->session->userdata('level') == 5) { ?>

                <a class="btn btn-info mt-1" href="<?= base_url('Aksi/complete/myhajat/lainnya/' . $data->id_myhajat_lainnya) ?>">Complete</a>
                <a class="btn btn-danger mt-1" href="<?= base_url('Aksi/reject/myhajat/lainnya/' . $data->id_myhajat_lainnya) ?>">Reject</a>
              <?php } ?>
            </div>
          </div>
        </div>
        ...
        <div class="col-lg-6">
          <div id="upload" class="card">
            <div class="card-header">
              <h3 class="card-title text-center">Lampiran (Attachment) <a class="btn btn-info float-right" href="<?= base_url('zip/createzip/tb_my_hajat_lainnya/id_myhajat_lainnya/myhajat/' . $data->id_myhajat_lainnya) ?>">Download All</a></h3>
            </div>
            ...
            <?php if (($this->session->userdata('level') == 1 || $this->session->userdata('level') == 6) && ($data->id_approval == 0 || $data->id_approval == 1)) { ?>
              <div class="card-footer text-center">
                <!-- Tombol ini muncul khusus untuk user -->
                <!-- <button type="button" id="ubah" class="btn btn-secondary">Ubah Data</button> -->
                <button type="submit" id="edit_lainnya" class="btn btn-info enable" name="edit_lainnya" disabled>Update Data!</button>
              </div>
            <?php } ?>
            <?php if ($this->session->userdata('level') == 5) { ?>
              <div class="card-footer text-center">
                <!-- Tombol ini muncul khusus untuk SUPERUSER -->
                <button type="submit" id="edit_lainnya_superuser" class="btn btn-info enable" name="edit_lainnya_superuser" disabled>Update Data!</button>
              </div>
            <?php } ?>
          </div>
        </div>
