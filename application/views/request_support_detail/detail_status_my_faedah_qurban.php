      ...
      <div class="row">
          ...
          <div class="card">
            ...
            <div class="card-body">
              ...
              <div class="form-group">
                ...
                <input type="hidden" class="form-control" name="id_qurban" id="id_qurban" value="<?= $data->id_qurban ?>" readonly required>
              </div>
              <!-- ID Vendor -->
              <input type="hidden" class="form-control id_vendor" name="id_vendor_qurban" id="id_vendor_qurban" value="<?= $data->id_vendor ?>" readonly required>
              <!-- Nama Cabang -->
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  <input name="nama_penyedia_qurban" type="text" class="form-control nama_vendor_myfaedah" value="<?= $data->nama_penyedia ?>" readonly required>
              ...
              <div class="form-group">
                ...
                <select class="form-control jenis-penyedia" name="jenis_penyedia_qurban" id="jenis_penyedia_qurban" disabled required>
              ...
              <div class="form-group">
                ...
                <input name="lama_usaha_qurban" id="lama_usaha_qurban" type="text" class="form-control enable" value="<?= $data->lama_usaha ?>" readonly required placeholder="Lama Usaha">
              </div>
              <!-- Jenis Hewan -->
              <div class="form-group">
                <label for="jenis_hewan_qurban">Jenis Hewan</label>
                <input name="jenis_hewan_qurban" id="jenis_hewan_qurban" type="text" class="form-control enable" value="<?= $data->jenis_hewan ?>" required readonly placeholder="Jenis Hewan">
              </div>
              <!-- Jumlah Hewan -->
              <div class="form-group">
                <label for="jumlah_hewan_qurban">Jumlah Hewan</label>
                <input name="jumlah_hewan_qurban" id="jumlah_hewan_qurban" type="number" class="form-control enable" value="<?= $data->jumlah_hewan ?>" required readonly placeholder="Jumlah Hewan">
              </div>
              <!-- Tujuan Pembelian -->
              <div class="form-group">
                <label for="tujuan_pembelian">Tujuan Pembelian</label>
                <div class="form-check form-check-info">
                  <label class="form-check-label"><input class="kategori form-check-input enable" value="Konsumtif" <?= $data->tujuan_pembelian == 'Konsumtif' ? 'checked' : '' ?> id="konsumtif" type="radio" name="tujuan_pembelian_qurban" value="Konsumtif" disabled>Konsumtif</label>
                </div>
                <div class="form-check form-check-info">
                  <label class="form-check-label"><input class="kategori form-check-input enable" value="Produktif" <?= $data->tujuan_pembelian == 'Produktif' ? 'checked' : '' ?> id="produktif" type="radio" name="tujuan_pembelian_qurban" value="Produktif" disabled>Produktif</label>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input name="nilai_pembiayaan_qurban" id="nilai_pembiayaan_qurban" type="number" class="form-control enable" value="<?= $data->nilai_pembiayaan ?>" required readonly placeholder="Jumlah Biaya">
                </div>
              </div>
              <!-- Informasi Tambahan -->
              <div class="form-group">
                <label for="informasi_tambahan_qurban">Informasi Tambahan</label>
                <textarea name="informasi_tambahan_qurban" id="informasi_tambahan_qurban" cols="30" rows="10" class="form-control enable" disabled><?= $data->informasi_tambahan ?></textarea>
              </div>
            ...
            <div class="card-footer">
              ...
              <?php if ($this->session->userdata('level') == 2 && ($data->id_approval == 0 || $data->id_approval == 4)) { ?>
                ...
                <a class="btn btn-info" onclick="return confirm('Apakah Anda yakin MENYETUJUI request support ini?')" href="<?= base_url('Aksi/approve/myfaedah/qurban/' . $data->id_qurban) ?>">Approve</a>
                <a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/myfaedah/qurban/' . $data->id_qurban) ?>">Reject</a>
              <?php } ?>
              <?php if ($this->session->userdata('level') == 3 && $data->id_approval == 2) { ?>

                <a class="btn btn-info" onclick="return confirm('Apakah Anda yakin MENYELESAIKAN request support ini?')" href="<?= base_url('Aksi/complete/myfaedah/qurban/' . $data->id_qurban) ?>">Approve</a>
                <a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/myfaedah/qurban/' . $data->id_qurban) ?>">Reject</a>
              <?php } ?>
              <!-- Tombol Aksi ini akan muncul untuk Admin Superuser -->
              <?php if ($this->session->userdata('level') == 5) { ?>

                <a class="btn btn-info mt-1" href="<?= base_url('Aksi/complete/myfaedah/qurban/' . $data->id_qurban) ?>">Complete</a>
                <a class="btn btn-danger mt-1" href="<?= base_url('Aksi/reject/myfaedah/qurban/' . $data->id_qurban) ?>">Reject</a>
              <?php } ?>
            </div>
          </div>
        ...
        <div class="col-lg-6">
          ...
          <div id="upload" class="card">
            <div class="card-header">
              <h3 class="card-title text-center">Lampiran (Attachment) <a class="btn btn-info float-right" href="<?= base_url('zip/createzip/tb_my_faedah_qurban/id_qurban/myfaedah/' . $data->id_qurban) ?>">Download All</a></h3>
            </div>
            ...
            <?php if (($this->session->userdata('level') == 1 || $this->session->userdata('level') == 6) && ($data->id_approval == 0 || $data->id_approval == 1)) { ?>
              <div class="card-footer text-center">
                <!-- Tombol ini muncul khusus untuk user -->
                <!-- <button type="button" id="ubah" class="btn btn-secondary">Ubah Data</button> -->
                <button type="submit" id="edit_qurban" class="btn btn-info enable" name="edit_qurban" disabled>Update Data!</button>
              </div>
            <?php } ?>
            <?php if ($this->session->userdata('level') == 5) { ?>
              <div class="card-footer text-center">
                <!-- Tombol ini muncul khusus untuk SUPERUSER -->
                <button type="submit" id="edit_qurban_superuser" class="btn btn-info enable" name="edit_qurban_superuser" disabled>Update Data!</button>
              </div>
            <?php } ?>
          </div>
        </div>
    ...
    <?php if ($komentar->num_rows() == 0) { ?>
      ...
      <div class="row mt-4">
        <div class="col-lg-12 col-md-6">
          <form method="post" action="<?= base_url('comment/post_comment/id_qurban') ?>">
            <div class="card">
              ...
              <div class="card-body">
                <div class="form-group">
                  ...
                  <input type="hidden" name="id_komentar" value="<?= $data->id_qurban ?>">
  ...
  <div class="row mt-4">
    <div class="col-lg-12 col-md-12">
      ...
      <div class="card card-widget">
        ...
        <div class="card-footer">
          <!-- Reply comment form -->
          <form action="<?= base_url('comment/post_reply/id_qurban'); ?>" method="post">
            <div class="img-push">
              <input name="parent_comment" type="hidden" value="<?= $komen->id ?>">
              <input type="hidden" name="id_user" value="<?= $this->fungsi->user_login()->id_user ?>">
              <input name="id_komentar" type="hidden" value="<?= $data->id_qurban ?>">
              <input type="hidden" name="redirect" value="<?= $this->uri->uri_string() ?>">
