			...
			<div class="row">
					...
					<div class="card">
						...
						<div class="card-footer">
							...
							<?php if ($this->session->userdata('level') == 2 && ($data->id_approval == 0 || $data->id_approval == 4)) { ?>
								...
								<a class="btn btn-info" onclick="return confirm('Apakah Anda yakin menyetujui request support?')" href="<?= base_url('Aksi/approve/mysafar/id/' . $data->id_mysafar) ?>">Approve</a>
								<a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/mysafar/id/' . $data->id_mysafar) ?>">Reject</a>
							<?php } ?>
							<?php if ($this->session->userdata('level') == 3 && $data->id_approval == 2) { ?>

								<a class="btn btn-info" onclick="return confirm('Apakah Anda yakin MENYELESAIKAN request support ini?')" href="<?= base_url('Aksi/complete/mysafar/id/' . $data->id_mysafar) ?>">Approve</a>
								<a class="btn btn-danger" onclick="return confirm('Apakah Anda yakin MENOLAK request support ini?')" href="<?= base_url('Aksi/reject/mysafar/id/' . $data->id_mysafar) ?>">Reject</a>
							<?php } ?>
							<!-- Tombol Aksi ini akan muncul untuk Admin Superuser -->
							<?php if ($this->session->userdata('level') == 5) { ?>

								<a class="btn btn-info mt-1" href="<?= base_url('Aksi/complete/mysafar/id/' . $data->id_mysafar) ?>">Complete</a>
								<a class="btn btn-danger mt-1" href="<?= base_url('Aksi/reject/mysafar/id/' . $data->id_mysafar) ?>">Reject</a>
							<?php } ?>
						</div>
					</div>
				</div>
				...
				<div class="col-lg-6">
					...
					<div id="upload" class="card">
						<div class="card-header">
							<h3 class="card-title text-center">Lampiran (Attachment) <a class="btn btn-info float-right" href="<?= base_url('zip/createzip/tb_my_safar/id_mysafar/mysafar/' . $data->id_mysafar) ?>">Download All</a></h3>
						</div>
